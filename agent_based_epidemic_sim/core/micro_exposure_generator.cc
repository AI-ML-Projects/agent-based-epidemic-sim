...
Exposure MicroExposureGenerator::Generate(
    const absl::Time& start_time,  // unused
    const absl::Duration& duration, float infectivity, float symptom_factor) {
  ...
  std::cout << duration << std::endl;
  std::cout << micro_exposure_counts << std::endl;
  return {
      .duration = duration,
